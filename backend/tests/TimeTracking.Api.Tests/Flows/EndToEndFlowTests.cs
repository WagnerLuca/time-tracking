...
public class EndToEndFlowTests : IClassFixture<TimeTrackingApiFactory>
{
    ...
    public async Task UserLifecycle_RegisterToDeleteAccount()
    {
        ...
        var settingsResp = await client.PutAsJsonAsync("/api/v1/Organizations/lifecycle-org/settings", new
        {
        ...
        // 4. Track time
        var startResp = await client.PostAsJsonAsync("/api/v1/TimeTracking/start", new
        {
        ...
        var stopResp = await client.PostAsJsonAsync("/api/v1/TimeTracking/stop", new
        {
        ...
        // 5. View history
        var historyResp = await client.GetAsync("/api/v1/TimeTracking");
        ...
        // 6. Change password
        var pwResp = await client.PostAsJsonAsync("/api/v1/Auth/change-password", new
        {
        ...
        // 7. Delete account
        var deleteResp = await client.DeleteAsync("/api/v1/Auth/account");
        deleteResp.EnsureSuccessStatusCode();

        // 8. Verify can't login
        TestHelpers.ClearAuth(client);
        var failedLogin = await client.PostAsJsonAsync("/api/v1/Auth/login", new
        {
    ...
    public async Task MultiUserOrganization_FullFlow()
    {
        ...
        // Owner adds employee
        var addResp = await ownerClient.PostAsJsonAsync("/api/v1/Organizations/multi-user-org/members", new
        {
        ...
        await empClient.PostAsJsonAsync("/api/v1/TimeTracking/start", new
        {
            description = "Employee working",
            organizationSlug = "multi-user-org"
        });
        await empClient.PostAsJsonAsync("/api/v1/TimeTracking/stop", (object?)null);

        // Enable member time entry visibility and work schedule changes
        await ownerClient.PutAsJsonAsync("/api/v1/Organizations/multi-user-org/settings", new
        {
            memberTimeEntryVisibility = true,
        ...
        var overviewResp = await ownerClient.GetAsync("/api/v1/Organizations/multi-user-org/time-overview");
        ...
        var entriesResp = await ownerClient.GetAsync(
            $"/api/v1/Organizations/multi-user-org/member-entries/{empLogin.User.Id}");
        ...
        var scheduleResp = await ownerClient.PostAsJsonAsync(
            $"/api/v1/organizations/multi-user-org/members/{empLogin.User.Id}/work-schedules", new
            {
    ...
    public async Task JoinRequestFlow_RequiresApproval_CompleteJourney()
    {
        ...
        await ownerClient.PutAsJsonAsync("/api/v1/Organizations/join-flow-org/settings", new { joinPolicy = 1 });
        ...
        var reqResp = await userClient.PostAsJsonAsync("/api/v1/organizations/join-flow-org/requests", new
        {
            type = 0,
        ...
        var myReqResp = await userClient.GetAsync("/api/v1/organizations/my-requests");
        ...
        var notifResp = await ownerClient.GetAsync("/api/v1/organizations/notifications");
        ...
        var acceptResp = await ownerClient.PutAsJsonAsync(
            $"/api/v1/organizations/join-flow-org/requests/{request!.Id}", new { accept = true });
        acceptResp.EnsureSuccessStatusCode();

        // User can now track time for the org
        var startResp = await userClient.PostAsJsonAsync("/api/v1/TimeTracking/start", new
        {
        ...
        await userClient.PostAsJsonAsync("/api/v1/TimeTracking/stop", (object?)null);
    }
    ...
    public async Task HolidayAndAbsenceFlow()
    {
        ...
        await ownerClient.PostAsJsonAsync("/api/v1/Organizations/hab-org/members", new
        {
        ...
        // Owner adds holiday
        var holResp = await ownerClient.PostAsJsonAsync("/api/v1/organizations/hab-org/holidays", new
        {
            date = "2026-12-25",
            name = "Christmas",
            isRecurring = true
        ...
        var presetResp = await ownerClient.PostAsync(
            "/api/v1/organizations/hab-org/holidays/import-preset?preset=de&year=2026", null);
        presetResp.EnsureSuccessStatusCode();

        // Member creates an absence
        var absResp = await memClient.PostAsJsonAsync("/api/v1/organizations/hab-org/absences", new
        {
            date = "2026-12-30",
            type = 1, // Vacation
            note = "Year-end break"
        ...
        var allAbsResp = await ownerClient.GetAsync("/api/v1/organizations/hab-org/absences");
        ...
        var adminAbsResp = await ownerClient.PostAsJsonAsync("/api/v1/organizations/hab-org/absences/admin", new
        {
            userId = memLogin.User.Id,
            date = "2026-12-31",
            type = 2, // Other
    ...
    public async Task RoleBasedAccessControl_EnforcesPermissions()
    {
        ...
        await ownerClient.PostAsJsonAsync("/api/v1/Organizations/rbac-org/members", new
        {
        ...
        var updateResp = await memberClient.PutAsJsonAsync("/api/v1/Organizations/rbac-org", new
        {
            name = "Hacked!"
        });
        ...
        var settingsResp = await memberClient.PutAsJsonAsync("/api/v1/Organizations/rbac-org/settings", new
        {
        ...
        var overviewResp = await memberClient.GetAsync("/api/v1/Organizations/rbac-org/time-overview");
        overviewResp.StatusCode.Should().Be(HttpStatusCode.Forbidden);

        // Member can't create holidays
        var holResp = await memberClient.PostAsJsonAsync("/api/v1/organizations/rbac-org/holidays", new
        {
            date = "2026-01-01",
            name = "Fake Holiday",
            isRecurring = false
        ...
        var absResp = await memberClient.PostAsJsonAsync("/api/v1/organizations/rbac-org/absences/admin", new
        {
            userId = memberLogin.User.Id,
            date = "2026-01-01",
            type = 0
        ...
        var reqResp = await memberClient.GetAsync("/api/v1/organizations/rbac-org/requests");
        reqResp.StatusCode.Should().Be(HttpStatusCode.Forbidden);

        // Member CAN create their own absence
        var ownAbsResp = await memberClient.PostAsJsonAsync("/api/v1/organizations/rbac-org/absences", new
        {
            date = "2026-05-01",
            type = 0
        });
        ...
        var startResp = await memberClient.PostAsJsonAsync("/api/v1/TimeTracking/start", new
        {
            organizationSlug = "rbac-org"
        });
        startResp.StatusCode.Should().Be(HttpStatusCode.Created);
        await memberClient.PostAsJsonAsync("/api/v1/TimeTracking/stop", (object?)null);
    }
    ...
    public async Task TokenRefreshChain_MultipleCycles()
    {
        ...
        // First refresh
        var refresh1 = await client.PostAsJsonAsync("/api/v1/Auth/refresh", new
        {
        ...
        var refresh2 = await client.PostAsJsonAsync("/api/v1/Auth/refresh", new
        {
